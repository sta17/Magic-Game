...
## Main [Equipment] node path.
## The main [Equipment] is used in most handler functions as a default inventory.
@export_node_path("Inventory") var equipment_path := NodePath("Equipment")

## Main [Equipment] node.
@onready var equipment := get_node(equipment_path)
...
## Returns [code]true[/code] if main [Inventory] is open.
func is_open_main_equipment() -> bool:
	return is_open(equipment)
...
## Open main [Equipment]. Return [code]true[/code] if opened successfully.
func open_main_equipment() -> bool:
	return open(equipment)


## Close main [Equipment]. Return [code]true[/code] if closed successfully.
func close_main_equipment() -> bool:
	return close(equipment)
...
func _instantiate_dropped_item(dropped_item : PackedScene,item : InventoryItem, amount: int):
	var pickUPItem = load("res://objects/items/PickUpItem.tscn")
	var pickUPItemObj = pickUPItem.instantiate()
	var obj = dropped_item.instantiate()
	pickUPItemObj.AddMeshModel(obj)
	drop_parent.add_child(pickUPItemObj)
	pickUPItemObj.position = get_parent().position
	pickUPItemObj.rotation = get_parent().rotation
	pickUPItemObj.item = item
	pickUPItemObj.amount = amount
	emit_signal("dropped", pickUPItemObj)
