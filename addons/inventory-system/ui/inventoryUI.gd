...
func _on_slot_gui_input(event : InputEvent, slot_obj):
	if event is InputEventMouseButton and event.double_click:
		if event.pressed:	
			...
			if index < 0:
				return 
			emit_signal("slot_point_down", event, index, inventory,true)
	elif event is InputEventMouseButton or event.is_action("ui_accept"):
		if event.pressed:	
			var index = slots.find(slot_obj)
			if index < 0:
				return 
			emit_signal("slot_point_down", event, index, inventory,false)
